...
def test_joint_model_arithmetic_and_weights(monkeypatch):
    ...
    assert round(out["allocation_pct"], 4) == 0.0475
    assert round(out["pred_home_score"], 4) == round(148.1 * (0.5 + 0.0475), 4)
    assert round(out["pred_away_score"], 4) == round(148.1 * (0.5 - 0.0475), 4)
