...
def _critical_for(path_str: str) -> bool:
    return path_str in CRITICAL_PATHS
...
for path_str, expected in registry.items():
    ...
    is_critical = _critical_for(path_str)
    if not path.exists():
        drifts.append(
            {
                "file": path_str,
        ...
        drifts.append(
            {
                ...
                "type": "COLUMN_DROPPED",
                "severity": "CRITICAL" if is_critical else "WARNING",
                "detail": f"{col} was present, now missing",
            }
        )
...
for d in drifts:
    msg = f"  [{d['severity']}] {d['file']} | {d['type']}: {d['detail']}"
